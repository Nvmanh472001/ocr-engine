...
from tools.utility import AverageMeter, Config, ConfigArguments
...
class Trainer(object):
    def __init__(
        self,
        cfg: Config,
        mode: Literal["train", "eval", "train_eval", "test"] = "train",
    ):
        self.cfg = cfg.cfg
        ...
        if hasattr(self.post_process_class, "character"):
            ...
            self.cfg["Architecture"]["Head"]["out_channels"] = char_num
        ...
        if self.train_dataloader:
            if (pruning_cfg := self.cfg["Slim"]["Pruning"]) is not None:
                self.oto = OTO(
                    model=self.model,
                    dummy_input=torch.rand(pruning_cfg["dummy_input"]),
                    strict_out_nodes=pruning_cfg["strict_out_nodes"],
                )

                self.oto.mark_unprunable_by_param_names(pruning_cfg["mark_unprunable"])

                # self.oto.random_set_zero_groups()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, epoch_num + 1):
            ...
            for idx, batch in enumerate(self.train_dataloader):
                ...
                # logger
                stats = {k: float(v) if v.shape == [] else v.detach().cpu().numpy().mean() for k, v in loss.items()}
...
def parse_args():
    parser = ConfigArguments()
    # parser.add_argument("--eval", action="store_true", default=True, help="Whether to perform evaluation in train")
...
def main():
    ...
    cfg.merge_dict(opt)
    trainer = Trainer(cfg, mode="train_eval")
    trainer.train()
